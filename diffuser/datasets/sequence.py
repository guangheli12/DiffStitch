...
class CondSequenceDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx, eps=1e-4):
        ...
        t_step = 1
        conditions = np.ones((self.horizon, self.observation_dim * 2)).astype(np.float32)       
        ...
        import random 
        start_index = random.randint(1, self.horizon - 2) 
        end_index   = random.randint(start_index, self.horizon - 2) 

        # [0, t_step] and [start_index, end_index] is masked 
        conditions[start_index : end_index + 1, :self.observation_dim] = 0 
        conditions[start_index : end_index + 1, self.observation_dim:] = observations[start_index : end_index + 1]   
        trajectories = np.concatenate([actions, observations], axis=-1) # shape is (100, 14)
